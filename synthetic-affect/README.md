...
- **The prediction-1 harness** — the loop against six stateless baselines on a
  six-task scripted suite, controls and loop-losing tasks included. Loop 5/6;
  best stateless 2/6; one task provably unsolvable without state; one task the
  loop honestly loses. [`experiments/results/RESULTS.md`](experiments/results/RESULTS.md)
